//compact: user profile view + editor (avatar, cover, socials, education, hobbies)
...
export default function Profile( ) {
  ...
  // local editable state (each field mirrors currentUser initially)
...
const [socialLinks, setSocialLinks]   = useState(currentUser?.socialLinks || {
 facebook: "", linkedin: "", twitter: "", github: ""
  ...
  // previews for profile + cover ( keeps UI snappy while file uploads happen later )
...
const [coverPreview, setCoverPreview] = useState(
 ...
 const [isLoading, setIsLoading] = useState(false);
  ...
  // keep local state in sync when currentUser updates
    // Tip- this keeps the editor fresh if the auth context updates elsewhere.
  useEffect(( ) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault( )
     setIsLoading (true)
        // optimistic UI: show a message and close editor when done
    try {
      await updateUser (
        displayName,
        profilePicFile,
...
const handleCoverFile = e => {
  ...
  // Dropzone: drag-and-drop for the avatar - small UX win
const onDrop = useCallback((acceptedFiles) => {
  ...
  // cleanup object URLs when component unmounts or preview changes (avoid memory leaks)
  useEffect( ( ) => {
    return ( ) => {
...
//language tag input--
  ...
  // social inputs config (keeps JSX tidy)
const socialConfig = [
  ...
  // education helpers - small utilities to mutate nested state safely
        ...
        { isEditing && (
          <>
            ...
            { isDragActive ? (
              ...
              <Camera className={styles.cameraIcon} size={25}/>
            )}
          </>
        )}
      </div>
...
<div className={styles.formGroup}>
  ...
  <div className={styles.detailItem}>
    ...
    <span className={styles.detailValue}>
      {
      socialConfig
        .map(({ key, Icon }) => ({ key, Icon, url: socialLinks[key] } ) )

        .filter(item => item.url)   // only show configured links
        .map(({ key, Icon, url }) => (
          ...
          >
            <Icon size={16} className={styles.socialIcon} />
