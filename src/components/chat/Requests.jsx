// handles incoming friend requests and displays your current friends
...
const FriendRequestsPanel = ({ onClose }) => {
  ...
  const [friends, setFriends] = useState ([]);   // accepted list 

  //  subscribe to any new friend requests for this user
  useEffect(( ) => {
    ...
    // Tip: always clean up your listeners or your app will leak memory and maybe kittens
    return () => unsubscribe();
  }, [currentUser.uid]);
  ...
  // Action: accept a friend request and remove it from the queue
  const acceptRequest = async (requesterUid) => {
    try {

        // Note- mutual friendship creation
  ...
  // Todo: could show a fancy toast here instead of alert and console.error
